...
async function renderGrid() {
  ...
  const ranked = await getRankedTeams(eventId);
  const event  = await getEvent(eventId);
  ...
  ranked.forEach((team, index) => {
    grid.appendChild(buildTeamCard(team, index + 1, event));
  });
}

function buildTeamCard(team, rank, event) {
  ...
  card.querySelectorAll('[data-delete-player]').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      await deletePlayer(eventId, team.id, btn.dataset.deletePlayer);
      refresh();
...
async function saveTeam() {
  ...
  if (editingTeamId) {
    await updateTeam(eventId, editingTeamId, { name, description: desc, score });
    toast('Équipe mise à jour.', 'success');
  } else {
    await addTeam(eventId, { name, description: desc, score });
...
async function savePlayer() {
  ...
  if (editingPlayerId) {
    await updatePlayer(eventId, editingTeamIdForPlayer, editingPlayerId, { name, description: desc });
    toast('Joueur mis à jour.', 'success');
  } else {
    await addPlayer(eventId, editingTeamIdForPlayer, { name, description: desc });
...
async function saveScore() {
  ...
  await updateTeam(eventId, scoreTeamId, { score });
...
function bindListeners() {
  ...
  document.getElementById('player-detail-edit-btn')?.addEventListener('click', async () => {
    const event  = await getEvent(eventId);
  ...
  // Supprimer équipe
  document.getElementById('confirm-delete-team-btn')?.addEventListener('click', async () => {
    if (!deleteTeamId) return;
    await deleteTeam(eventId, deleteTeamId);
    deleteTeamId = null;
